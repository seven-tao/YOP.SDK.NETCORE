...
namespace SDK.yop.utils
{
    public sealed class SHA1withRSA
    {
        ...
        public static string sign(string content, string privateKey, string input_charset)
        {
            ...
            byte[] dataBytes = Encoding.GetEncoding(input_charset).GetBytes(content);
            RSACryptoServiceProvider rsa = DecodePemPrivateKey(privateKey);
            var signatureBytes = _privateKeyRsaProvider.SignData(dataBytes, HashAlgorithmName.SHA256, RSASignaturePadding.Pkcs1);
